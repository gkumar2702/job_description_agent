"""
Simple test for JDParser without dependencies.
"""
...
class JDParser:
    ...
    def _extract_company(self, text: str, subject: str, sender: str) -> str:
        ...
        # Normalize text by replacing Unicode quotes and spaces
        ...
        # Try simpler patterns if no match found
        simple_patterns = [
            r'at\s+([A-Z][a-zA-Z\s&.,\-]+(?:\s+[A-Z][a-zA-Z\s&.,\-]+)+?)(?:\s*\.|$|\n)',  # Match multi-word company names
    ...
    def _extract_role(self, text: str, subject: str) -> str:
        ...
        for pattern in self.role_patterns:
            match = re.search(pattern, text + " " + subject, re.IGNORECASE)
            if match:
...
def test_jd_parser():
    ...
    assert jd1.company == "Acuity Knowledge Partners"
    assert jd1.role == "Lead Data Scientist"
    assert jd1.location == "Bangalore"
    assert jd1.experience_years == 7
    assert jd1.salary_lpa == 50.0
    assert "Python" in jd1.skills
    assert "Machine Learning" in jd1.skills
    ...
    assert jd2.company == "UST"
    assert jd2.role == "AI Engineer"
    assert jd2.location == "Bangalore"
    assert jd2.experience_years == 5
    assert "Python" in jd2.skills
    assert "AWS" in jd2.skills
